...
除原有的空间、设备、任务、遭遇、玩家流程之外，本包新增两项职责，见 [README](README.md#本包按-v20-新增的两项职责)：**地图生成的底层逻辑**归本包，**运行期由本包调用模型 Adapter** 读取对方模组已加载的资源。MAP2 与 MAP9 因此要重新定范围，新增 MAP-GEN 与 MAP-ADAPTER 两个批次。
...
仍未做的三项：与 D3 对接真实创建上下文和 source evidence——无法证实的映射明确标未知，不借诊断的反射结果直接登记 game-verified；通过公开注册路径暴露首批已核验的读取能力；执行 `tests/fixtures/native-identity-scenarios.json` 里的十个原生身份规格。

退出验收：多维度同编号、同资源多实例、生成重入、销毁后的迟到回调、同 ID 新生命都不混淆；无私有的跨模块引用。

## MAP2 — Room 空间、布局与拓扑

开始条件：MAP1 完成，网站可导出统一的 room / source / placement 合同。

消费真实的房间定义和子房间来源，保留变换、marker 与 plug、权威部件层级及实例；**不从整房或整组的预览模型推导可执行布局**。实现固定 seed 的候选选择与布局约束，检查端口类型、朝向、双面门与额外连接、碰撞与重叠、跨区连通和局部导航；检查结果带具体的对象引用。区分生成前的静态检查、游戏生成阶段、NavMesh 就绪与可用空间发布——生成未完成时不能把采样成功记作房间可用。保留合法空间的查询入口给 encounter、deployable 和玩家落点；Enemy 提供所需的 clearance、movement 与 collision，不由 Room 自动塞入敌人数量。

房间的权威数据是游戏原生的部件层级、局部变换、实例、Area 与连接器与碰撞与导航与遮挡及引用；多个房间以实例与拓扑组成房间组。共享网格与纹理字节可以去重，**资源身份、授权与实例仍然独立**。

退出验收：端口类型不匹配、朝向错误、重叠、跨区不连通、NavMesh 未就绪各有确定结果，且不把静态检查通过当作生成成功。

## MAP-GEN — 地图生成的底层逻辑（新增，F9）

开始条件：MAP2 的空间与拓扑合同稳定；网站侧确定了玩家拼装结果的表示形式。

这是总案第 4.2 节对本包新增的核心职责。生成逻辑要**一次实现、对所有资源来源通用**：房间与房间组的实例化、局部变换与轴向与尺度、Area 与连接器与门插口的建立、碰撞与导航与遮挡的接入、实例与拓扑的登记。逐家 Geo 包的写法差异全部收敛到资源侧 Adapter，不进入这一层。

生成过程必须可复现：固定 seed、显式预算、稳定排序、每一步的失败原因可追踪。生成不足时返回明确的部分结果和原因，**不自动把剩余对象堆到任意位置**。

退出验收：同一份拼装结果在同一 seed 下生成完全相同的世界；换一家资源来源不需要改动本层任何代码。

## MAP-ADAPTER — 模型 Adapter 的运行期调用（新增，F9）

开始条件：MAP-GEN 的生成逻辑可用；网站侧完成编辑期的提取与拼装。

Adapter 只负责"资源怎么读出来"这一层：从对方模组**已在游戏内加载的**资源里，按已核验的资源协议或 profile 取得部件、层级、局部变换、材质与灯光引用。按已核验的协议或 profile 复用，**不逐内容包或逐编辑器增加 Provider**。原版 GearPart、GPC 部件、enemy prefab、geomorph 与自有资源加载器各自保留真实身份，**不按包名猜兼容**。
...
退出验收：至少三家不同作者的 Geo 包走同一条流程；新接一个包只写一个资源侧 Adapter，MAP-GEN 一行不改。
