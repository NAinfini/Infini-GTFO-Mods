...
// Nothing is loaded into or executed by GTFO; virtual dispatch, list membership and call order are not resolved.
if (args.Length != 5)
{
    Console.Error.WriteLine("Usage: MapNativeEvidence <BepInEx> <GTFO game root> <dump.cs> <hook-spec.json> <report.json>");
    return 2;
}
...
try
{
    ...
    TypeDefinition? Type(JsonElement row) => assemblies[row.GetProperty("assembly").GetString()!].MainModule.Types.SelectMany(Walk)
        .SingleOrDefault(t => t.FullName == row.GetProperty("type").GetString());
    ...
    foreach (var hook in spec.GetProperty("hooks").EnumerateArray())
    {
        ...
        var matches = Type(hook)?.Methods.Where(m => m.Name == name).ToArray() ?? Array.Empty<MethodDefinition>();
    ...
    // Readbacks are interop members the plugin calls; only their managed signatures are frozen here.
    foreach (var readback in spec.GetProperty("readbacks").EnumerateArray())
    {
        string id = readback.GetProperty("id").GetString()!, signature = readback.GetProperty("signature").GetString()!;
        var matches = Type(readback)?.Methods.Where(m => m.FullName == signature).ToArray() ?? Array.Empty<MethodDefinition>();
        Check("readback." + id, matches.Length == 1 && matches[0].IsStatic == readback.GetProperty("isStatic").GetBoolean(), signature);
    }
...
Console.WriteLine($"{(failed == 0 ? "PASS" : "FAIL")} {checks.Count - failed}/{checks.Count} Map static native evidence checks; game execution NOT tested.");
return failed == 0 ? 0 : 1;
