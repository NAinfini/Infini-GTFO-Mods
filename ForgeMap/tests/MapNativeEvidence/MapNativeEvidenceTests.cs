...
namespace ForgeMap.Tests.MapNativeEvidence;
...
public sealed class MapNativeEvidenceTests
{
    ...
    private static void RequireInputs()
    {
        Assert.False(string.IsNullOrWhiteSpace(BepInEx) || string.IsNullOrWhiteSpace(GameRoot) || string.IsNullOrWhiteSpace(Dump),
            "Set FORGE_MAP_BEPINEX (or GTFO_BEPINEX_PATH), FORGE_GTFO_GAME and FORGE_GTFO_DUMP to existing local inputs.");
    }

    [Fact]
    public void map_static_native_evidence_checks_all_pass()
    {
        RequireInputs();
        var checks = Audit();
        ...
        Assert.True(checks.Count >= 15, "Expected the full evidence set, observed " + checks.Count + " checks.");
    }
    ...
    private static List<EvidenceCheck> Audit()
    {
        ...
        try
        {
            ...
            foreach (var hook in spec.GetProperty("hooks").EnumerateArray())
            {
                ...
                Check("signature." + id, matches.Length == 1 && matches[0].FullName == hook.GetProperty("signature").GetString()
                    && matches[0].IsVirtual == hook.GetProperty("isVirtual").GetBoolean() && matches[0].IsStatic == hook.GetProperty("isStatic").GetBoolean(),
                    string.Join(" | ", matches.Select(m => m.FullName + (m.IsVirtual ? " [virtual]" : "") + (m.IsStatic ? " [static]" : ""))));
