...
public sealed class Plugin : BasePlugin
{
    ...
    {
        PluginLog = Log;
        // Remove retired persisted options before binding; BepInEx otherwise keeps orphan entries.
        if (System.IO.File.Exists(Config.ConfigFilePath))
        {
            string previous = System.IO.File.ReadAllText(Config.ConfigFilePath);
            string current = MarkerRules.CleanConfig(previous);
            if (current != previous)
            {
                System.IO.File.WriteAllText(Config.ConfigFilePath, current);
                Config.Reload();
            }
        }
        Settings.Bind(Config);
        CombatStatistics.Initialize(Config);
        ...
        Telemetry.SnapshotRequested += ItemMarkers.LogPerformance;
