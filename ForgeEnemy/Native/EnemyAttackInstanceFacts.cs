...
/// <summary>The two facts a received damage transaction settles, read on the one window that already spans it.
///
...
///
/// Nothing here writes the world: both facts are readings of a commit the game already made.</summary>
internal sealed partial class EnemyModule
{
    /// <summary>The two bindings this family publishes, in registration order. They are the same two ids
    /// <see cref="ForgeEnemy.AttackInstanceContract"/> declares for the registration rows, so the publication
    /// gate and the registry can never name different rows.</summary>
    ...
    private void PublishKilled(Entry entry)
    {
        ...
        PublishAttackInstance("gtfo.enemy.kill", AttackInstanceContract.BindingId, entry.Reference, ports);
    }
    ...
    private void PublishLimbDamaged(Entry entry, DamageInstanceObservation before, double healthAfter)
    {
        ...
        if (amount <= 0) return;
        PublishAttackInstance("gtfo.enemy.limb", AttackInstanceContract.LimbDamagedBindingId, entry.Reference,
    ...
    private void PublishAttackInstance(string scope, string binding, EntityReference subject, Dictionary<string, object> ports)
    {
        if (!CanPublishDamageInstance || Resolve(subject) == null) return;
        ...
        if (result.Status == "rejected") _report("attack instance fact rejected: " + result.Code);
    }
