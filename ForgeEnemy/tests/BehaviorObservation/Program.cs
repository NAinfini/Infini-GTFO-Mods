...
// Module registration and plan loading are frozen at startup, so a started scene is the default here.
Scene Started() { var scene = new Scene(); scene.Start(); return scene; }
...
{
    using var s = Started(); var before = s.Enemy.Damage.Sends; _ = s.Read();
...
{
    using var s = Started(); var token = s.Module.CaptureDespawn(s.Enemy); s.Module.CompleteDespawn(token);
...
{
    using var s = Started(); s.Kernel.BeginWorld(2); var fresh = s.Module.TrackSpawn(s.Enemy);
...
{
    using var s = Started(); s.Allowed = false; Require(s.Read() == null, "Phase gate was bypassed.");
});
Case("gate.client-denied", () =>
{
    using var s = Started(); SNet.IsMaster = false; Require(s.Read() == null, "Client read claimed host behavior authority.");
});
Case("lifecycle.dead-unavailable", () =>
{
    using var s = Started(); s.Enemy.Alive = false; Require(s.Read() == null, "Dead AI was exposed as active behavior.");
});
Case("components.missing-ai", () =>
{
    using var s = Started(); s.Enemy.AI = null!; Require(s.Read() == null, "Missing AI fabricated a snapshot.");
});
Case("components.missing-locomotion", () =>
{
    using var s = Started(); s.Enemy.Locomotion = null!; Require(s.Read() == null, "Missing locomotion fabricated a snapshot.");
});
Case("components.missing-abilities", () =>
{
    using var s = Started(); s.Enemy.Abilities = null!; Require(s.Read() == null, "Missing abilities fabricated a snapshot.");
});
Case("components.owner-mismatch", () =>
{
    using var s = Started(); s.Enemy.Locomotion.m_agent = Scene.NewEnemy(8, 20);
...
{
    using var s = Started(); s.Enemy.OnInvisibleRead = () => throw new IOException("synthetic native getter");
    Require(s.Read() == null, "Native getter exception escaped the observation boundary.");
});
Case("lifecycle.pre-start", () =>
{
    using var s = new Scene(); Require(s.Read() == null, "Pre-start behavior was accepted.");
});
Case("lifecycle.stopped", () =>
{
    using var s = Started(); s.Kernel.StopRuntime(); Require(s.Read() == null, "Stopped runtime returned behavior.");
});
Case("snapshot.frozen", () =>
{
    using var s = Started(); var x = s.Read()!; s.Enemy.Locomotion.CurrentStateEnum = ES_StateEnum.Dead;
    Require(x.NativeState == (int)ES_StateEnum.PathMove, "Snapshot retained mutable native state.");
});
Case("thread.off-thread-rejected", () =>
{
    using var s = Started(); Exception? caught = null;
...
{
    using var s = Started(); s.Enemy.m_hasValidTarget = false; s.Enemy.Abilities.ActiveAbility = AgentAbility.None;
...
rows.AddRange(BehaviorFacts.Run().Select(x => new Row(x.Id, x.Passed, x.Detail)));

int failed = rows.Count(x => !x.Passed);
var report = new { schemaVersion = 1, verification = "E4-domain-native-behavior-observation-and-behaviour-facts-with-game-doubles",
    factBindings = EnemyModule.BehaviorBindings.ToArray(),
    gameExecuted = false, multiplayerExecuted = false,
    passed = rows.Count - failed, failed, checks = rows };
