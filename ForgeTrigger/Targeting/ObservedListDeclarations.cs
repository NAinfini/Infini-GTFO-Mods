...
public static class ObservedListDeclarations
{
    ...
    public static ObservedFamily Family { get; } = ObservedFamily.Declare(
        ...
        ObservedDeclaration.Node("forge.condition.list.contains", "query", "列表包含", "指定的实体在这个候选列表里时通过。",
            ObservedDeclaration.Inputs(ObservedDeclaration.Many("items"), ObservedDeclaration.Entity("item")),
            ObservedDeclaration.Outputs(ObservedDeclaration.Bool("value")), ObservedDeclaration.NoParameters,
            new HandlerShape().Inputs("items", "item").Outputs("value"), ContainsHandler, new[] { "world" }),
